...
export const InvitationsModal = ({ onClose }: InvitationsModalProps) => {
    ...
    const load = async () => {
      ...
      try {
        const res = await fetch(
          `${configManager.functionsBaseUrl}/list-roles?application_id=${configManager.authAppId}&api_key=${configManager.authApiKey}`
        );
  ...
  const loadInvitations = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${configManager.functionsBaseUrl}/invitations-list`, {
        method: 'POST',
  ...
  const sendInvitation = async () => {
    ...
    setSendSuccess('');
    try {
      const res = await fetch(`${configManager.functionsBaseUrl}/invitations-create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          application_id: configManager.authAppId,
  ...
  const revokeInvitation = async (invitationId: string) => {
    ...
    try {
      const res = await fetch(`${configManager.functionsBaseUrl}/invitations-revoke`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          application_id: configManager.authAppId,
