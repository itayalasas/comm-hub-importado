...
export default function Documentation({ publicView = false }: DocumentationProps) {
  ...
  const endpoints: EndpointSection[] = [
      ...
      responses: [
        {
          ...
          example: {
            ...
            pdf_public_url: 'https://api.tu-dominio.com/view-pdf?token=abc123',
    ...
    },
    {
      ...
      requestBody: {
        ...
        schema: {
          ...
          kind: 'scheduled | batch (required)',
          delivery_mode: 'static | queued (optional)',
          channel: 'email | email_pdf | pdf',
          template_name: 'string (conditional)',
          pdf_template_name: 'string (conditional)',
          pdf_filename_pattern: 'string (optional)',
          recipients: 'array (required for static, optional for queued)',
          schedule_at: 'string ISO 8601 (required for scheduled, optional for batch)',
          timezone: 'string (optional)',
          shared_data: 'object (optional)',
          options: 'object (optional)',
          cron_expression: 'string (optional, cron de recurrencia para scheduled)',
        },
        example: {
      ...
      responses: [
        {
          code: '201',
          description: 'Programacion creada',
          example: {
            success: true,
            data: {
              program: {
                id: 'uuid-programa',
                status: 'scheduled',
                schedule_at: '2026-06-30T18:00:00Z',
                next_run_at: '2026-06-30T18:00:00Z',
                cron_expression: '0 9 * * 1-5',
              },
            },
          },
        },
        {
          ...
          example: {
            success: false,
            error: { code: 'VALIDATION_ERROR', message: 'schedule_at es requerido para programas programados' },
          },
        },
      ],
    },
    {
      id: 'automation-programs-queue-create',
      title: 'Encolar Items',
      method: 'POST',
      path: '/automation-programs/:program_id/queue',
      description: 'Agrega uno o varios items a la cola de una programacion concreta. Cada item queda ligado al programa y a la aplicacion.',
      authentication: 'API Key (x-api-key header)',
      headers: [
        { name: 'x-api-key', type: 'string', required: true, description: 'API key de la aplicacion' },
        { name: 'Content-Type', type: 'string', required: true, description: 'application/json' },
      ...
      parameters: [
        { name: 'program_id', type: 'string', required: true, description: 'ID de la programacion' },
      ],
      requestBody: {
        contentType: 'application/json',
        schema: {
          external_reference_id: 'string (optional, recomendado)',
          recipient_email: 'string (required)',
          recipient_data: 'object (optional)',
          shared_data: 'object (optional)',
          options: 'object (optional)',
          available_at: 'string ISO 8601 (optional)',
          items: 'array (optional para enviar varios items)',
        },
        example: {
          items: [
            {
    ...
    {
      id: 'automation-programs-queue-list',
      title: 'Listar Cola',
      method: 'GET',
      path: '/automation-programs/:program_id/queue',
      description: 'Recupera los items encolados de una programacion y permite filtrarlos por estado.',
      authentication: 'API Key (x-api-key header)',
      headers: [
        { name: 'x-api-key', type: 'string', required: true, description: 'API key de la aplicacion' },
      ],
      parameters: [
        { name: 'program_id', type: 'string', required: true, description: 'ID de la programacion' },
        { name: 'status', type: 'string', required: false, description: 'queued, processing, sent, failed o cancelled' },
        { name: 'limit', type: 'number', required: false, description: 'Cantidad maxima de items' },
      ],
      responses: [
        {
          code: '200',
          description: 'Listado de cola',
          example: {
            success: true,
            data: {
              program: {
    ...
    {
      id: 'automation-programs-queue-delete',
      title: 'Cancelar Item de Cola',
      method: 'DELETE',
      path: '/automation-programs/:program_id/queue/:queue_item_id',
      ...
      parameters: [
        { name: 'program_id', type: 'string', required: true, description: 'ID de la programacion' },
        { name: 'queue_item_id', type: 'string', required: true, description: 'ID del item de cola' },
      ],
      responses: [
        {
          code: '200',
    ...
    {
      id: 'automation-programs-list',
      title: 'Listar Programaciones',
      method: 'GET',
      path: '/automation-programs',
      description: 'Recupera las programaciones del tenant actual. Puedes filtrar por tipo, estado o por las que ya estan listas para ejecutarse.',
      authentication: 'API Key (x-api-key header)',
      headers: [
        { name: 'x-api-key', type: 'string', required: true, description: 'API key de la aplicacion' },
      ],
      parameters: [
        { name: 'kind', type: 'string', required: false, description: 'scheduled o batch' },
        { name: 'status', type: 'string', required: false, description: 'scheduled, active, paused, done, failed o cancelled' },
        { name: 'due', type: 'boolean', required: false, description: 'true para devolver solo programaciones vencidas' },
        { name: 'limit', type: 'number', required: false, description: 'Cantidad maxima de registros a retornar' },
      ],
      responses: [
        {
          code: '200',
          description: 'Listado de programaciones',
          example: {
            success: true,
            data: {
              programs: [
    ...
    {
      id: 'automation-programs-run',
      title: 'Ejecutar Programacion',
      method: 'POST',
      path: '/automation-programs/:program_id/run',
      description: 'Dispara manualmente una programacion guardada. Si usa cola externa, procesa los items pendientes asociados a esa programacion antes de cerrar la ejecucion.',
      authentication: 'API Key (x-api-key header)',
      headers: [
        { name: 'x-api-key', type: 'string', required: true, description: 'API key de la aplicacion' },
      ],
      parameters: [
        { name: 'program_id', type: 'string', required: true, description: 'ID de la programacion a ejecutar' },
      ],
      responses: [
        {
          code: '200',
        ...
        {
          code: '404',
          description: 'Programacion no encontrada',
          example: {
            success: false,
    ...
    },
    {
      ...
      title: 'Monitoreo de Automatizaciones',
      method: 'GET',
      path: '/automation-monitoring',
      description: 'Devuelve el resumen operativo de programaciones, cola externa, jobs recientes y trazas cronologicas para diagnostico rapido. Incluye la proxima ejecucion de cada programa.',
      authentication: 'API Key (x-api-key header)',
      headers: [
        { name: 'x-api-key', type: 'string', required: true, description: 'API key de la aplicacion' },
      ],
      parameters: [
        { name: 'limit', type: 'number', required: false, description: 'Cantidad maxima de elementos por seccion' },
      ],
      responses: [
        {
          code: '200',
          description: 'Snapshot operativo',
          example: {
            success: true,
            data: {
    ...
    },
    {
      ...
      headers: [
        ...
        { name: 'x-scheduler-token', type: 'string', required: false, description: 'Token alternativo para cron jobs' },
        { name: 'Content-Type', type: 'string', required: true, description: 'application/json' },
      ],
      requestBody: {
        contentType: 'application/json',
      ...
      responses: [
        {
          code: '200',
          description: 'Programaciones procesadas',
          example: {
            success: true,
            data: {
              claimed: 2,
        ...
        },
        {
          ...
          example: {
            success: false,
            error: { code: 'UNAUTHORIZED', message: 'Invalid scheduler token' },
          },
        },
      ],
    },
    ...
    headers.forEach((header) => {
      const headerValue =
        header.name.toLowerCase() === 'x-api-key'
          ...
          ? 'Bearer scheduler_token_xxxxxxxxx'
          : header.name.toLowerCase() === 'x-scheduler-token'
          ? 'scheduler_token_xxxxxxxxx'
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="text-slate-300 space-y-2 text-sm">
              ...
              <li>• Programaciones automáticas y ejecución en lote</li>
            </ul>
          </div>
        </div>
      </div>
