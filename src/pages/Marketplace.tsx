...
const DEFAULT_BASE_URL = 'https://api.tu-dominio.com';

const getBaseUrl = () => {
  const { functionsBaseUrlRaw, functionsBaseUrl } = getRuntimeConfig();
  return functionsBaseUrlRaw || functionsBaseUrl || DEFAULT_BASE_URL;
};
...
const buildConnectors = (BASE_URL: string): ConnectorManifest[] => [
  ...
  },
  {
    ...
    tagline: 'Programacion y ejecucion de envios',
    description: 'Crea, lista y ejecuta programaciones para envios futuros o reutilizables desde tu CRM. Pensado para flujos que necesitan una primera fecha, repeticion con cron, seguimiento operativo y cola externa.',
    category: 'automation',
    version: '1.0.0',
    author: 'SendCraft',
    icon: 'automation',
    ...
    stats: [
      ...
      { label: 'Endpoint', value: 'POST /automation-programs' },
      { label: 'Auth', value: 'API Key' },
    ],
    auth: {
      type: 'api_key',
      header: 'x-api-key',
      label: 'API Key de la aplicacion',
      placeholder: 'sk_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
      hint: 'La misma API Key de tu aplicacion en SendCraft',
    },
    baseUrl: BASE_URL,
    actions: [
      {
        ...
        description: 'Crea una programacion para ejecutar mas adelante o reutilizarla luego. Puede trabajar con recipients fijos o recibir items externos ligados al programa y a la aplicacion.',
        method: 'POST',
        endpoint: '/automation-programs',
        ...
        params: [
          { key: 'name', field_path: 'name', label: 'Nombre', type: 'string', required: true, placeholder: 'Lanzamiento semanal' },
          { key: 'delivery_mode', field_path: 'delivery_mode', label: 'Modo', type: 'select', required: false, options: [
            { value: 'static', label: 'Datos fijos' },
            { value: 'queued', label: 'Cola externa' },
          ] },
          { key: 'channel', field_path: 'channel', label: 'Canal', type: 'select', required: true, options: [
            { value: 'email', label: 'Email' },
            { value: 'email_pdf', label: 'Email + PDF' },
            { value: 'pdf', label: 'PDF' },
          ] },
          { key: 'template_name', field_path: 'template_name', label: 'Template email', type: 'string', required: false, placeholder: 'welcome' },
          { key: 'pdf_template_name', field_path: 'pdf_template_name', label: 'Template PDF', type: 'string', required: false, placeholder: 'factura_pdf' },
          { key: 'schedule_at', field_path: 'schedule_at', label: 'Primera ejecucion', type: 'string', required: true, placeholder: '2026-06-30T15:00:00-03:00' },
          { key: 'cron_expression', field_path: 'cron_expression', label: 'Cron recurrente', type: 'string', required: false, placeholder: '0 9 * * 1-5' },
          { key: 'recipients', field_path: 'recipients', label: 'Destinatarios', type: 'array', required: false, placeholder: '[{"email":"cliente@ejemplo.com"}]' },
        ],
        request_example: {
          name: 'Lanzamiento semanal',
          kind: 'scheduled',
          delivery_mode: 'queued',
          channel: 'email_pdf',
          template_name: 'welcome',
          pdf_template_name: 'factura_pdf',
          schedule_at: '2026-06-30T15:00:00-03:00',
          cron_expression: '0 9 * * 1-5',
        },
      },
      {
        id: 'run_program',
        name: 'Ejecutar programacion',
        description: 'Dispara una programacion guardada y crea el job asociado. Si usa cola externa, consume los items pendientes de ese programa.',
        method: 'POST',
        endpoint: '/automation-programs/:program_id/run',
        params: [
          { key: 'program_id', field_path: 'path.program_id', label: 'ID de la programacion', type: 'string', required: true, placeholder: 'uuid-programa' },
        ],
        request_example: {
          url: `${BASE_URL}/automation-programs/uuid-programa/run`,
      ...
      },
      {
        ...
        name: 'Encolar items',
        description: 'Envía uno o varios items a la cola de una programacion especifica. Cada item queda asociado a la aplicacion y al programa.',
        method: 'POST',
        endpoint: '/automation-programs/:program_id/queue',
        params: [
          { key: 'program_id', field_path: 'path.program_id', label: 'ID de la programacion', type: 'string', required: true, placeholder: 'uuid-programa' },
          { key: 'recipient_email', field_path: 'recipient_email', label: 'Email destino', type: 'string', required: true, placeholder: 'cliente@ejemplo.com' },
          { key: 'external_reference_id', field_path: 'external_reference_id', label: 'Referencia externa', type: 'string', required: false, placeholder: 'order-123' },
        ],
        request_example: {
          items: [
            {
      ...
      },
      {
        ...
        name: 'Listar cola',
        description: 'Recupera los items encolados de una programacion, filtrando por estado si hace falta.',
        method: 'GET',
        endpoint: '/automation-programs/:program_id/queue',
        params: [
          { key: 'program_id', field_path: 'path.program_id', label: 'ID de la programacion', type: 'string', required: true, placeholder: 'uuid-programa' },
          { key: 'status', field_path: 'query.status', label: 'Estado', type: 'string', required: false, placeholder: 'queued' },
          { key: 'limit', field_path: 'query.limit', label: 'Limite', type: 'string', required: false, placeholder: '20' },
        ],
        request_example: {
          url: `${BASE_URL}/automation-programs/uuid-programa/queue?status=queued&limit=20`,
          headers: { 'x-api-key': 'sk_xxx' },
        },
      },
      {
        ...
        name: 'Cancelar item',
        description: 'Cancela un item puntual sin borrar la programacion completa.',
        method: 'DELETE',
        endpoint: '/automation-programs/:program_id/queue/:queue_item_id',
        params: [
          { key: 'program_id', field_path: 'path.program_id', label: 'ID de la programacion', type: 'string', required: true, placeholder: 'uuid-programa' },
          { key: 'queue_item_id', field_path: 'path.queue_item_id', label: 'ID del item', type: 'string', required: true, placeholder: 'uuid-item' },
        ],
        request_example: {
          url: `${BASE_URL}/automation-programs/uuid-programa/queue/uuid-item`,
          headers: { 'x-api-key': 'sk_xxx' },
        },
      },
      {
        ...
        description: 'Recupera las programaciones activas, pausadas o programadas para el tenant actual.',
        method: 'GET',
        endpoint: '/automation-programs',
        params: [
          { key: 'kind', field_path: 'query.kind', label: 'Tipo', type: 'string', required: false, placeholder: 'scheduled' },
          { key: 'status', field_path: 'query.status', label: 'Estado', type: 'string', required: false, placeholder: 'scheduled' },
          { key: 'limit', field_path: 'query.limit', label: 'Limite', type: 'string', required: false, placeholder: '20' },
        ],
        request_example: {
          url: `${BASE_URL}/automation-programs?kind=scheduled&limit=20`,
          headers: { 'x-api-key': 'sk_xxx' },
        },
      },
    ],
    features: [
      ...
      'Ejecucion manual o desde un scheduler externo',
      'Cola externa por programa y por aplicacion',
      'Historial de jobs asociado a cada programacion',
      'Canales email, email + PDF y PDF',
      'Pensado para CRM, backoffice y automatizaciones',
    ],
  },
  {
    id: 'sendcraft-monitoring',
    name: 'SendCraft Monitoring',
    tagline: 'Trazas y estadisticas operativas',
    description: 'Consulta el estado de programas, jobs y trazas para automatizaciones y lotes. Ideal para dashboards internos, alertas y seguimiento de ejecuciones.',
    category: 'automation',
    version: '1.0.0',
    author: 'SendCraft',
    icon: 'automation',
    ...
    stats: [
      { label: 'Version', value: '1.0.0' },
      { label: 'Endpoint', value: 'GET /automation-monitoring' },
      { label: 'Auth', value: 'API Key' },
    ],
    auth: {
      type: 'api_key',
    ...
    actions: [
      {
        id: 'get_snapshot',
        name: 'Obtener snapshot',
        description: 'Devuelve resumen de programas, jobs recientes y trazas operativas.',
        method: 'GET',
        endpoint: '/automation-monitoring',
        params: [
          { key: 'limit', field_path: 'query.limit', label: 'Limite', type: 'string', required: false, placeholder: '25' },
        ],
        request_example: {
          url: `${BASE_URL}/automation-monitoring?limit=25`,
          headers: { 'x-api-key': 'sk_xxx' },
        },
      },
    ],
    features: [
      'Resumen de programas, jobs y aperturas/clicks',
      'Trazas cronologicas para diagnostico rapido',
      'Proxima ejecucion y ultimo error por programa',
      'Vista compacta para dashboards internos',
      'Soporta integracion con alertas externas',
      'Ideal para monitoreo de lote y programacion',
    ],
  },
  {
            ...
            <div className="bg-slate-800/30 rounded-xl border border-slate-700/50 px-4 py-3 space-y-2">
              ...
              <div className="flex items-center gap-2 bg-slate-950/50 border border-slate-700/50 rounded-lg px-3 py-2">
                <code className="flex-1 text-[11px] text-cyan-300 break-all">
                  {`${connector.baseUrl}/connectors/${connector.id}`}
                </code>
                <button
                  onClick={() => copy(`${connector.baseUrl}/connectors/${connector.id}`, 'url')}
