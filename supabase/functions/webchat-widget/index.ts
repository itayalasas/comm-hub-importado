...
async function upsertConversation(
  ...
  const incomingMessage = firstString(body.message, body.assistant_reply, body.assistantReply);
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    logWidgetEvent("POST start", {
      action: firstString(body.action).toLowerCase() || "message",
      sessionId,
      conversationId,
      tenantKey: identity.tenantKey,
      scopeKey: identity.scopeKey,
    ...
    if (firstString(body.action).toLowerCase() === "request_agent_handoff") {
